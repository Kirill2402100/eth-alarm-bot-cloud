# scanner_engine.py
# ============================================================================
# v32.0 - TIME-BASED ANALYSIS
# - Логика полностью переработана для анализа СТАБИЛЬНОСТИ уровней.
# - Бот теперь входит в сделку, только если крупный ордер в стакане
#   удерживается дольше заданного времени (MIN_WALL_STABILITY_SEC).
#   Это позволяет торговать на более "старшем таймфрейме".
# ============================================================================
import asyncio
import time
import logging
...
# === Конфигурация сканера (РЕЖИМ АКТИВНЫЙ ИНТРАДЕЙ) ======================
...
# --- НОВЫЙ КЛЮЧЕВОЙ ПАРАМЕТР "ТАЙМФРЕЙМА" ---
MIN_WALL_STABILITY_SEC = 90  # Стена считается надежной, если стоит дольше 90 секунд
...
# === Логика сканирования (ПОЛНОСТЬЮ ПЕРЕРАБОТАНА) ==========================
async def scan_for_new_opportunities(exchange, app: Application, broadcast_func):
    ...
    try:
        ...
        # 1. Обновляем информацию о стабильности стен
        now = time.time()
        current_support, current_resistance = large_bids[0], large_asks[0]
        stable_walls = bot_data.get('stable_walls', {})

        # Проверяем стену поддержки
        if abs(stable_walls.get('support', {}).get('price', 0) - current_support['price']) > 10: # Если цена сместилась
            stable_walls['support'] = {'price': current_support['price'], 'seen_since': now}
        
        # Проверяем стену сопротивления
        if abs(stable_walls.get('resistance', {}).get('price', 0) - current_resistance['price']) > 10: # Если цена сместилась
            stable_walls['resistance'] = {'price': current_resistance['price'], 'seen_since': now}
        
        bot_data['stable_walls'] = stable_walls

        # 2. Ищем сигнал, опираясь на стабильные стены
        dominant_side_is_bids = top_bids_usd > top_asks_usd
        
        # Проверяем LONG сигнал
        support_stability = now - stable_walls.get('support', {}).get('seen_since', now)
        if dominant_side_is_bids and support_stability > MIN_WALL_STABILITY_SEC:
            side = "LONG"
            stable_wall = stable_walls['support']
        # Проверяем SHORT сигнал
        elif not dominant_side_is_bids and (now - stable_walls.get('resistance', {}).get('seen_since', now)) > MIN_WALL_STABILITY_SEC:
            side = "SHORT"
            stable_wall = stable_walls['resistance']
        else:
            status_code = "WAIT_STABILITY"
            status_message = f"Поиск стабильного уровня... S: {support_stability:.0f}с, R: {now - stable_walls.get('resistance', {}).get('seen_since', now):.0f}с"
            return

        # 3. Стабильный уровень найден, ищем агрессию
