...
save_state(ctx.application)
await update.message.reply_text(f"✅ Бот v{BOT_VERSION} запущен.\nИспользуйте /run для запуска.", parse_mode=constants.ParseMode.HTML)

async def cmd_stop(update:Update, ctx:ContextTypes.DEFAULT_TYPE):
ctx.bot_data["bot_on"] = False
save_state(ctx.application)
await update.message.reply_text("🛑 Бот остановлен.", parse_mode=constants.ParseMode.HTML)
...
msg = (f"Состояние бота v{BOT_VERSION}\n"
f"Статус: {'✅ ON' if bot_data.get('bot_on') else '🛑 OFF'}\n"
f"Основной цикл: {'🚀 RUNNING' if is_running else '🔌 STOPPED'}\n"
f"Диагностика: {'ВКЛ' if bot_data.get('debug_mode_on') else 'ВЫКЛ'}\n"
f"Активных сделок: {len(active_signals)}\n"
f"Депозит: ${bot_data.get('deposit', 50)}\n"
f"Плечо: x{bot_data.get('leverage', 100)}\n")
if active_signals:
signal = active_signals[0]
msg += f"Активная сделка: {signal.get('Pair')} {signal.get('side')}\n"
...
save_state(ctx.application)
await update.message.reply_text(f"✅ Депозит установлен: ${new_deposit}", parse_mode=constants.ParseMode.HTML)
...
save_state(ctx.application)
await update.message.reply_text(f"✅ Плечо установлено: x{new_leverage}", parse_mode=constants.ParseMode.HTML)
except (IndexError, ValueError):
