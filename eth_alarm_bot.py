#!/usr/bin/env python3
# -*- coding: utf-8 -*-
# ssl-rsi okx swap bot

import os, json, math, asyncio, signal
...
assert BOT_TOKEN and CHAT_IDS and SHEET_ID, "⛔ Set BOT_TOKEN, CHAT_IDS, SHEET_ID !"
...
def ssl_channel(df):
    ...
    for i in range(1,len(df)):
        p,c = up[i-1],dn[i-1], up[i],dn[i]
        if pd.notna(c[0]):
            if p[0]<p[1] and c[0]>c[1]: sig.append("LONG")
            elif p[0]>p[1] and c[0]<c[1]: sig.append("SHORT")
...
async def market_df():
    ...
    df['rsi']=df.c.diff().pipe(lambda x:100-100/(1+ x.clip(lower=0).rolling(14).mean()/
                                                     -x.clip(upper=0).rolling(14).mean()))
    df['sig']=ssl_channel(df[['h','l','c']].rename(columns={'h':'high','l':'low','c':'close'}))
    return df
...
async def open_trade(side, price):
    ...
    return dict(side="LONG"if side=="buy"else"SHORT",entry=fill,sl=sl,tp=tp,
                qty=qty,time=datetime.now(timezone.utc))
...
async def cmd_stop(u:Update,c:ContextTypes.DEFAULT_TYPE):
    ...
    monitoring=False; position=None
    await exchange.close()                      # ← корректное закрытие
