...
# • CRITICAL FIX: Restructured `timeframes` map to be correctly nested under options['timeframes']['swap'].
# • CRITICAL FIX: Changed BTC market snapshot symbol to the correct unified swap symbol 'BTC/USDT:USDT'.
# • This version is expected to be the final, working version for MEXC.
# ============================================================================
...
# === State ================================================================
STATE_FILE = "bot_state_v5_1.json"
state = {}
def load_state():
    global state
...
# === Exchange & Strategy ==================================================
# ИЗМЕНЕНИЕ: Полностью исправленная конфигурация для MEXC SWAP
exchange = ccxt.mexc({
    'options': {
        'defaultType': 'swap',
        'timeframes': {
...
# ... (Остальная часть кода идентична последней рабочей версии)
...
async def get_market_snapshot():
    try:
        # ИЗМЕНЕНИЕ: Используем правильный символ для SWAP рынка
        ohlcv_btc = await exchange.fetch_ohlcv('BTC/USDT:USDT', '1d')
...
async def scanner(app):
    while state["bot_on"]:
        try:
            ...
            for c in pre:
                try:
                    df = pd.DataFrame(await exchange.fetch_ohlcv(c["pair"], TF_ENTRY, limit=100), columns=["timestamp", "open", "high", "low", "close", "volume"])
...
async def cmd_start(update:Update, ctx:ContextTypes.DEFAULT_TYPE):
    ...
    if not state["bot_on"]:
        ...
        await update.message.reply_text("✅ <b>Бот v5.1 (MEXC Final Fix) запущен.</b>"); 
...
if __name__=="__main__":
    ...
    log.info("Bot v5.1 (MEXC Final Fix) started.")
    app.run_polling()
