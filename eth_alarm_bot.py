...
async def run_searching_phase(app):
    ...
    if llm_response and llm_response.get('decision') == 'APPROVE':
        ...
        if setup:
            ...
            message = (
                f"🔔 <b>ЛУЧШИЙ СЕТАП!</b> 🔔\n\n"
                f"<b>Монета:</b> <code>{setup.get('pair')}</code>\n<b>Направление:</b> <b>{setup.get('side')}</b>\n"
                f"<b>Take Profit:</b> <code>{setup.get('tp')}</code>\n<b>Stop Loss:</b> <code>{setup.get('sl')}</code>\n\n"
                f"<b>Обоснование LLM:</b> <i>{setup.get('reason')}</i>\n\n"
