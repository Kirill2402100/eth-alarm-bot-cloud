...
async def get_market_snapshot():
    try:
        ...
        if len(ohlcv_btc) < 51:
            raise ValueError("Received less than 51 candles for BTC")
        df_btc = pd.DataFrame(ohlcv_btc, columns=["timestamp", "open", "high", "low", "close", "volume"])
        for col in ["open", "high", "low", "close", "volume"]:
            df_btc[col] = pd.to_numeric(df_btc[col])
        df_btc = add_indicators(df_btc)
        if df_btc.empty:
            raise ValueError("BTC DataFrame is empty after indicator calculation")
        ...
        try:
            ...
            for i, (sym, _) in enumerate(pairs):
                ...
                try:
                    ...
                    side = None
                    if adx < MIN_M15_ADX:
                        rejection_stats["LOW_ADX"] += 1
                        continue
                    side = None
                    if prev15["EMA_9"] <= prev15["EMA_21"] and last15["EMA_9"] > last15["EMA_21"]:
                        side = "LONG"
                    elif prev15["EMA_9"] >= prev15["EMA_21"] and last15["EMA_9"] < last15["EMA_21"]:
                        side = "SHORT"
                    if not side:
                        rejection_stats["NO_CROSS"] += 1
                        continue
                    if market_regime == "BEARISH" and side == "LONG":
                        rejection_stats["MARKET_REGIME"] += 1
                        continue
                    if (last15["high"]-last15["low"]) > last15[f"ATR_{ATR_LEN}"]*ANOMALOUS_CANDLE_MULT:
                        rejection_stats["ANOMALOUS_CANDLE"] += 1
                        continue
                    ...
                    for col in ["open", "high", "low", "close", "volume"]:
                        df1h[col] = pd.to_numeric(df1h[col])
            ...
            # ... (Rest of logic for LLM processing and signal generation)
        except Exception as e:
    ...
    while state["bot_on"]:
        ...
        for s in list(state["monitored_signals"]):
            try:
                ...
                hit=None
                exit_price = None
                if (s["side"]=="LONG" and price>=s["tp"]):
                    hit, exit_price = "TP_HIT", s["tp"]
                elif (s["side"]=="SHORT" and price<=s["tp"]):
                    hit, exit_price = "TP_HIT", s["tp"]
                elif (s["side"]=="LONG" and price<=s["sl"]):
                    hit, exit_price = "SL_HIT", s["sl"]
                elif (s["side"]=="SHORT" and price>=s["sl"]):
                    hit, exit_price = "SL_HIT", s["sl"]
                if hit:
...
if __name__=="__main__":
    ...
    if state.get("bot_on"):
        # This check is a bit redundant because post_init already handles it,
        # but it ensures the scanner starts immediately on run if it was already on,
