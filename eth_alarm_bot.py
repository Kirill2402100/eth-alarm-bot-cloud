# ============================================================================
#  eth_alarm_bot.py — Variant B   (22-Jun-2025)
...
ATR_THR      = 0.60         # 0.6 × ATR отклонение цены от cross_price
...
state = {
    ...
    "position": None,
    "pending":  None,       # dict при Cond 1+2 OK
    "last_ssl_sig": 0,
...
async def close_pos(reason:str, price:float, ctx):
    p = state['position'];  state['pending'] = None
    if not p: return
    order = await exchange.create_market_order(PAIR,
...
async def monitor(ctx):
    ...
    while True:
        ...
        try:
            ...
            backoff = min(backoff*2, max_back);  continue
        ...
        # ── 1) новое пересечение SSL → фиксируем cross_price ─────────────
        if sig != 0 and sig != state['last_ssl_sig']:
            state['last_ssl_sig'] = sig

            cross_price = df['close'].iloc[-2]          # закрытие последнего полного бара
            state['pending'] = dict(
                side        = 'LONG' if sig==1 else 'SHORT',
                cross_price = cross_price,
                cross_ts    = df['ts'].iloc[-2],
            )

            await broadcast(ctx,
                f"ℹ️ Cond 1+2 OK ({state['pending']['side']}) | "
                f"SSL {cross_price:.0f} | RSI {row['rsi']:.1f} | Δ 0.00 %")

        # ── 2) есть pending → ждём ценовой фильтр ────────────────────────
        pen = state['pending']
        if pen and state['position'] is None:
            delta_abs = abs(price - pen['cross_price'])
            delta_pct = delta_abs / price
            atr_cur   = row['atr']
            # Проверяем относительное отклонение к текущему ATR
            if delta_abs >= ATR_THR * atr_cur:
                await open_pos(pen['side'], price, ctx)
                state['pending'] = None
